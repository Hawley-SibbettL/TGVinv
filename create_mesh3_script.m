function [input,mesh]=create_mesh3(input,manual_flag)
% Note - manual flag edited to be new max_depth 

% save('CM1 inputs topography')
...
elseif manual_flag == 2
    ...
    mesh = find_max_n(input,mesh);
    max_depth = max(mesh.depth_n); clear mesh.max_n mesh.depth_n
    temp_depth = depth_calculator(max_depth,mesh.probe_spacing,input.refine_mesh_flag);
    mesh.depth_n = temp_depth;
    mesh.max_n = length(mesh.depth_n);
end
% save('CM2 find_max_n','mesh')
mesh=calc_param(input,mesh);
% save('CM3 calc_param','mesh')
mesh=create_mesh_gen(input,mesh);
% save('CM4 create_mesh_gen','mesh')
% calculate_half_band
mesh=create_mesh_grid(input,mesh);
% save('CM5 create_mesh_grid','mesh')
[input,mesh] = geometrical_factor(input,mesh);
% save('CM6 geometrical_factor','input','mesh')
...
function [input,mesh] =geometrical_factor(input,mesh)
% in


% include first topography in ax and az coordinates
% if input.topography_flag == 1
%     load('anaglyfo_data')
%     for i=1:input.num_mes
%         for j=1:length(anaglyfo_data(:,1))
...
if input.time_lapse_flag==1
    ...
    if size(ind,1)~=0 && size(ind,2)~=-0
        display('Negative resistivitys in measurements');
        ind
        pause
    end   
...
elseif input.time_lapse_flag==0
    ...
    mesh.mean_res = mean(input.real_data);
end
...
if input.topography_flag == 1
    % Assume all electrodes are surface (all z variation is topography)
    tmp_sur = tmp_ALL;
    tmp_bor = [];
...
else % Original code to detect boreholes
...
% Refine grid to half electrode spacing if flag = 1
if input.refine_mesh_flag == 1   
    add_x1 = zeros(1,2*(length(add_x_points))-1);
    add_x1(1:2:end) = add_x_points;
    add_x1(2:2:end) = (add_x_points(2:end) + add_x_points(1:end-1))/2;
    add_x_points = add_x1;
end
...
% Find probe coordiantes in numbering    
    ...
    mesh.orig_probe_z2=mesh.orig_probe_z;
    mesh.orig_probe_z=0*mesh.orig_probe_z;
...
tmp1=0;
% calculates a parameter grid with no topography
for i=1:mesh.max_n-1    % For each layer in the parameter grid
...
tmp=1;
% first loop puts nodes at parameter corners and edge midpoints
for i=1:mesh.num_param
    
    % the corners of the parametr are nodes
    ...
    % Interpolation points (i need to add more elements within each
    % parameter
...
% Reads topography straight from input file z coordinates
% Better to add it in directly when specifying first nodes, as current
% form doesn't account for edge nodes... May also make refined mesh work
% better- or may need to extrapolate the topography vector
if input.topography_flag == 1
    
    anag_probe_ind = [];
    anaglyfo_data = [orig_probe_x, zeros(size(orig_probe_x))];
    
    % Find indices of electrodes and add the known z values at these
    % positions
    for q = 1:length(mesh.orig_probe_x)
        new_ind = find(anaglyfo_data(:,1) == mesh.orig_probe_x(q));
        if isempty(new_ind) == 0
            anag_probe_ind = [anag_probe_ind, new_ind];
            continue
        end
    end
    anaglyfo_data(anag_probe_ind,2) = mesh.orig_probe_z2;
    
    % Linearly interpolate z for other nodes (should be cell centroids)
    % Assume boundary topography is flat to 'infinity'
    anaglyfo_data(1,2) = mesh.orig_probe_z2(1);
    anaglyfo_data(end,2) = mesh.orig_probe_z2(end);
    for q = 2:(length(anaglyfo_data) - 1)
        if anaglyfo_data(q,2) == 0
            anaglyfo_data(q,2) = (anaglyfo_data(q-1,2) + anaglyfo_data(q+1,2))/2;
        end
    end
    
    anaglyfo_data(:,2) = anaglyfo_data(:,2) - mean(anaglyfo_data(:,2));

    
    % anaglyfo;
    
    % CURRENTLY ANALGYFO NOT WORKING
    % h=gcf;
    % uiwait(h);
    ...
    save('anaglyfo_data','anaglyfo_data')
